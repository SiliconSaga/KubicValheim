...
**The live event server is `play.terasology.org:32456`** (instance `valheim7`, world `Jotunheim`). That name is a single explicit **A record** on a domain carrying many unrelated records, pointed at one node's external IP — it is not managed by any ingress or Gateway, and it is not the `@`/`*` pair that nordri's `scripts/update-dns-namecheap.sh` rewrites. Do **not** point that script at this domain: it calls `namecheap.domains.dns.setHosts`, which replaces the domain's entire record set. Repointing this name means updating one A record **and** the `address=` in the `tafl` component's `kustomize/fleet/published-ip.yaml`. **Be precise about what that rule does: it never resolves DNS.** It checks only whether the address written in the rule still appears among the cluster's node `ExternalIP` metrics — so it catches the common failure, a node being recycled out from under a published address, and it cannot catch the A record and the rule disagreeing with each other. Update both together or the alert is watching an address nobody is being sent to, and it will sit green while the fleet is unreachable. Closing that gap needs a real DNS check (a blackbox probe against the hostname), which is not built. The rule is deliberately NOT in this repo: one A record serves every hosted game, so the drift is a fleet condition rather than a Valheim one, and a copy per instance produced a separate critical page per namespace for a single DNS failure.
